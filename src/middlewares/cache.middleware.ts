...
function generateCacheKey(req: Request): string {
  const tenantId = (req as TenantRequest).tenant?.id || 'public';
 ...
 */
const CACHE_CONFIG: { [key: string]: { ttl: number; tags: string[] } } = {
  // Stream endpoints
 ...
 */
function getCacheConfig(req: Request): { ttl: number; tags: string[] } | null {
...
export class CacheInvalidator {
   ...
   */
  static async invalidateStream(streamId: string, tenantId: string): Promise<void> {
   ...
   */
  static async invalidateUser(wallet: string, tenantId: string): Promise<void> {
