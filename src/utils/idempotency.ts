...
const recentOperations = new Map<string, CacheEntry>();
...
export function getRequestIdempotencyKey(
  ...
  return generateIdempotencyKey(
    ...
    req.method,
    req.path,
    bodyHash.substring(0, 50) // Use first 50 chars of body
  );
}
 ...
 */
export async function checkIdempotencyFast(
  key: string,
  ...
  if (useDbCache) {
    try {
      const dbCached = await executeQuery(
 ...
 */
export async function naturalIdempotentOperation<T>(
  operation: () => Promise<T>
): Promise<T> {
 ...
 */
export function clearAllIdempotencyCaches(): void {
