...
export function singletonController(controllerName: string, controller: Function) {
  return async (req: any, res: any, next?: any) => {
    ...
    console.log(`[Singleton-Queue] ${controllerName} request queued at ${requestStart}`);
    const executionKey = `${controllerName}_executed`;
    const executionId = Math.random().toString(36).substring(7);
    ...
    console.log(`[Singleton-Queue] ${controllerName} started after ${Date.now() - requestStart}ms wait`);
  };
}