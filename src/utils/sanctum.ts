...
export class SanctumGateway {
   ...
   */
  async buildGatewayTransaction(transaction: Transaction): Promise<string> {
    try {
  ...
  async submitTransaction(signedTransaction: Transaction): Promise<string> {
    try {
      ...
      if (this.fallbackConnection) {
        ...
        const signature = await this.fallbackConnection.sendRawTransaction(
   ...
   */
  async buildSignAndSubmit(
    unsignedTransaction: Transaction,
    signFunction: (tx: Transaction) => Promise<Transaction>
  ): Promise<string> {
    try {
...
// Singleton instance
let sanctumInstance: SanctumGateway | null = null;

export const getSanctumGateway = (connection?: Connection): SanctumGateway => {
  if (!sanctumInstance) {
