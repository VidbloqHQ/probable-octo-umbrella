  ...
  try {
    ...
    if (pollAgendas.length > 0) {
      const pollPromise = Promise.all(
        pollAgendas.map(async (pollAgenda) => {
    ...
    if (quizAgendas.length > 0) {
      const quizPromise = Promise.all(
        quizAgendas.map(async (quizAgenda) => {
          ...
          const created = await db.agenda.create({
            data: {
              ...
              quizContent: {
                create: {
                  questions: {
                    create: quizAgenda.questions.map((q: any) => ({
    ...
    if (customAgendas.length > 0) {
      const customPromise = Promise.all(
        customAgendas.map(async (customAgenda) => {
          const created = await db.agenda.create({
            data: {
