...
export const submitQuizAnswers = async (req: TenantRequest, res: Response) => {
  ...
  try {
    ...
    const quizQuestionIds = new Set(
      agenda.quizContent.questions.map((q) => q.id)
    );
    const invalidQuestionIds = answers.filter(
      (a) => !quizQuestionIds.has(a.questionId)
    );
...
export const getQuizResults = async (req: TenantRequest, res: Response) => {
  ...
  try {
    ...
    const [participants, allParticipantsCount, questionStats] =
      await Promise.all([
        executeQuery(
          () =>
        ...
        Promise.all(
          agenda.quizContent.questions.map(async (question) => {
    ...
    const leaderboard = participants.map((participant) => {
      ...
      const pointsFromQuiz = participant.quizResponses.reduce(
