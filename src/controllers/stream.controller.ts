 ...
 */
function getEnabledStreamTypes(tenant: any): string[] {
  const enabledTypes = [];
...
export const createStream = async (req: TenantRequest, res: Response) => {
  const {
    ...
    scheduledFor,
    title,
    streamSessionType, // Optional override to tenant default
  ...
  try {
    ...
    if (streamSessionType) {
      ...
      if (tenantWithDetails.enabledStreamTypes) {
        ...
        const isTypeEnabled = 
          (streamSessionType === StreamSessionType.Livestream && enabledTypes.enableStream) ||
      ...
      if (tenantWithDetails.enabledStreamTypes) {
        ...
        const isDefaultEnabled = 
          (resolvedStreamSessionType === StreamSessionType.Livestream && enabledTypes.enableStream) ||
