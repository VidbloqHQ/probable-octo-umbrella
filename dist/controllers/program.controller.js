...
const vidbloqIdl = JSON.parse(readFileSync(join(__dirname, "../../src/idl/vidbloq_program.json"), "utf-8"));
// Program ID from IDL
...
const initializeProgram = () => {
    ...
    return new Program(vidbloqIdl, provider);
};
// // Get PDA for a stream
const getStreamPDA = (streamName, hostPublicKey) => {
    const [pda] = PublicKey.findProgramAddressSync([Buffer.from("stream"), Buffer.from(streamName), hostPublicKey.toBuffer()], PROGRAM_ID);
    return pda;
};
// Get PDA for a donor account
const getDonorPDA = (streamPDA, donorPublicKey) => {
    const [pda] = PublicKey.findProgramAddressSync([Buffer.from("donor"), streamPDA.toBuffer(), donorPublicKey.toBuffer()], PROGRAM_ID);
    return pda;
};
// // Helper to deserialize instruction
const deserializeInstruction = (serializedIx) => {
    return {
        keys: serializedIx.keys.map((k) => ({
 ...
 */
export const getStreamPDAController = async (req, res) => {
    ...
    try {
        ...
        const pda = getStreamPDA(streamName, new PublicKey(hostPublicKey));
 ...
 */
export const buildInitializeTransactionController = async (req, res) => {
 ...
 */
export const buildStartStreamTransactionController = async (req, res) => {
 ...
 */
export const buildCompleteStreamTransactionController = async (req, res) => {
    const { streamName, hostPublicKey } = req.body;
    const tenant = req.tenant;
    try {
        // Tenant verification
 ...
 */
export const buildDepositTransactionController = async (req, res) => {
    const { streamName, hostPublicKey, donorPublicKey, amount, donorATA, streamATA, } = req.body;
    const tenant = req.tenant;
    try {
        // Tenant verification
        if (!tenant) {
 ...
 */
export const buildDistributeTransactionController = async (req, res) => {
    const { streamName, hostPublicKey, recipientPublicKey, amount, mintAddress, streamATA, recipientATA, } = req.body;
    const tenant = req.tenant;
    try {
        // Tenant verification
        if (!tenant) {
 ...
 */
export const buildRefundTransactionController = async (req, res) => {
    const { streamName, hostPublicKey, donorPublicKey, initiatorPublicKey, amount, donorATA, streamATA, } = req.body;
    const tenant = req.tenant;
    try {
        // Tenant verification
        if (!tenant) {
 ...
 */
export const buildUpdateStreamTransactionController = async (req, res) => {
    ...
    try {
        ...
        const ix = await program.methods
            .updateStream(newEndTime ? new BN(newEndTime) : null, newStatus ? statusData : null)
            .accounts({
            host: hostPubkey,
            stream: streamPDA,
        })
...
//  * This version handles confirmation timeouts more gracefully
//  */
...
//     }
//     // Verify transaction on Solana blockchain with retry logic
...
//         // Get signature status instead of confirming
//         const signatureStatus = await connection.getSignatureStatus(signature, {
//           searchTransactionHistory: true,
//         });
//         if (signatureStatus.value) {
//           const { confirmationStatus, err } = signatureStatus.value;
//           if (err) {
//             txError = err;
//             console.error('Transaction failed on blockchain:', err);
//             break;
//           }
...
//         }
//       } catch (err) {
//         console.error(`Error checking transaction status (attempt ${attempts}):`, err);
//         if (attempts >= maxAttempts) {
//           // Log the error but don't fail immediately
//           console.warn('Could not confirm transaction status, proceeding with caution');
//           break;
//         }
//         await new Promise(resolve => setTimeout(resolve, delayMs));
//       }
//     }
//     // If we have a transaction error, return it
//     if (txError) {
//       return res.status(400).json({ 
...
//       });
//     }
//     // If transaction is not confirmed after all attempts, log warning but proceed
//     if (!txConfirmed) {
//       console.warn(`Transaction ${signature} confirmation status unknown after ${maxAttempts} attempts`);
//       // You might want to store this transaction with a 'pending' status
//       // and have a background job to verify it later
//     }
...
//               isInitialized: true,
//               // Add a confirmation status field if needed
//               // confirmationStatus: txConfirmed ? 'confirmed' : 'pending'
//             },
//           });
//           if (!updatedRecord) {
...
//             },
//             data: {
//               status: txConfirmed ? "confirmed" : "pending",
//               transactionSignature: signature,
//               confirmedAt: txConfirmed ? new Date() : null,
//             },
//           });
//           if (!updatedRecord) {
...
//             },
//             data: {
//               refundConfirmed: txConfirmed,
//               refundedAt: txConfirmed ? new Date() : null,
...
// };
// /**
//  * Controller for fetching a stream
//  */
...
//       ...streamRecord,
//       totalDeposited: streamRecord.totalDeposited.toString(),
//       totalDistributed: streamRecord.totalDistributed.toString(),
//       donations: streamRecord.donations.map(donation => ({
//         ...donation,
//         // Convert any BigInt fields in donations if they exist
//       })),
//       distributions: streamRecord.distributions.map(distribution => ({
//         ...distribution,
//         // Convert any BigInt fields in distributions if they exist
//       })),
//     };
//     // Get on-chain data
//     try {
...
// };
// /**
//  * Controller for listing streams for a tenant
//  */
 ...
 */
export const listDonationsController = async (req, res) => {
    ...
    try {
        ...
        if (donorWallet) {
            filters.donorWallet = donorWallet;
        }
        // Parse pagination params
 ...
 */
export const listDistributionsController = async (req, res) => {
    ...
    try {
        ...
        if (recipientWallet) {
            filters.recipientWallet = recipientWallet;
        }
        // Parse pagination params
        const parsedLimit = limit ? parseInt(limit) : 10;
        const parsedOffset = offset ? parseInt(offset) : 0;
...
export const listStreamsController = async (req, res) => {
    const { hostWallet, status, limit, offset } = req.query;
    const tenant = req.tenant;
    try {
        // Tenant verification
...
export const recordTransactionController = async (req, res) => {
    const { signature, transactionType, streamId, donationId, distributionId, wallet } = req.body;
    const tenant = req.tenant;
    try {
        // Tenant verification
        if (!tenant) {
...
export const getStreamController = async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            const hostPubkey = new PublicKey(hostPublicKey);
            const streamPDA = getStreamPDA(streamName, hostPubkey);
            const onChainData = await program.account.streamState.fetch(streamPDA);
            // Return combined data
            return res.status(200).json({
                stream: serializedStream,
                onChainData: {
