...
async function forwardToTenant(tenant, event) {
    ...
    try {
        ...
        if (!res.ok) {
            console.error(`[WEBHOOK] Delivery failed for tenant ${tenant.id}: ${res.status} ${res.statusText}`);
        }
        else {
            console.log(`[WEBHOOK] Delivered ${event.type} to tenant ${tenant.id} (room: ${event.roomName})`);
        }
    }
    catch (err) {
        console.error(`[WEBHOOK] Delivery error for tenant ${tenant.id}:`, err instanceof Error ? err.message : err);
    }
}
// ============================================
    ...
    try {
        ...
        if (event.event === "participant_left") {
            ...
            if (participantIdentity) {
                await executeQuery(() => db.participant.updateMany({
                    where: {
...
export const registerWebhook = async (req, res) => {
    ...
    try {
        ...
        const webhookSecret = existingTenant?.webhookSecret || generateWebhookSecret();
