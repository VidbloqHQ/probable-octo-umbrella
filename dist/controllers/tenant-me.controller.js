...
export const updateTenant = async (req, res) => {
    try {
        ...
        const { name, theme, primaryColor, secondaryColor, logo, templateId, rpcEndpoint, networkCluster, defaultStreamType, defaultFundingType, enabledStreamTypes, authorizedDomains, // Array of domains to authorize
         } = req.body;
        ...
        await db.$transaction(async (tx) => {
            ...
            if (enabledStreamTypes !== undefined) {
                ...
                if (existingEnabledTypes) {
                    ...
                    await tx.enabledStreamTypes.update({
                        where: { tenantId },
                        data: {
                            enableStream: enabledStreamTypes.enableStream ??
                                existingEnabledTypes.enableStream,
                            enableMeeting: enabledStreamTypes.enableMeeting ??
                                existingEnabledTypes.enableMeeting,
                            enablePodcast: enabledStreamTypes.enablePodcast ??
            ...
            if (authorizedDomains && Array.isArray(authorizedDomains)) {
                ...
                if (domainsToAdd.length > 0) {
                    await Promise.all(domainsToAdd.map((domain) => tx.authorizedDomain.create({
                        data: {
                            domain,
                            tenantId,
                        },
