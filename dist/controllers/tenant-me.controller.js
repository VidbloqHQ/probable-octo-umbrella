...
export const updateTenant = async (req, res) => {
    ...
    try {
        ...
        if (authorizedDomains && Array.isArray(authorizedDomains)) {
            ...
            // Domains to remove
            const domainsToRemove = existingDomains.filter(d => !newDomainSet.has(d.domain));
            // Add new domains (parallel, independent operations)
            const addPromises = domainsToAdd.map(domain => executeQuery(() => db.authorizedDomain.create({
            ...
            const removePromises = domainsToRemove.map(d => executeQuery(() => db.authorizedDomain.delete({
                where: { id: d.id }
            }), { maxRetries: 1, timeout: 2000 }).catch(err => {
