...
// // ============================================
// app.post("/test-token-direct", async (req: Request, res: Response) => {
...
//       return res.status(401).json({ error: "Invalid API credentials" });
//     }
//     const { createStreamToken } = await import(
...
//         if (duration > 1000) {
//           console.warn(
//             `[SLOW] ${req.method} ${req.path} - ${res.statusCode} (${duration}ms)`
//           );
//         }
//       });
//     }
...
function logTiming(req, stage) {
    const reqId = req.id || `${Date.now()}-${Math.random()}`;
...
app.post('/test-auth-timing', async (req, res) => {
    const start = Date.now();
