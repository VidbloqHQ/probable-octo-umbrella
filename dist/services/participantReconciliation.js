        ...
        for (const stream of activeStreams) {
            ...
            for (const participant of activeParticipantsInDb) {
                ...
                if (isInactive) {
                    ...
                    await ParticipantManager.markParticipantAsLeft(stream.name, participant.walletAddress, participant.id);
            ...
            for (const participant of leftParticipants) {
                ...
                const isStillActiveById = Boolean(clientsByIdentity[participant.id] &&
                    activeParticipants[stream.name]?.[participant.id] &&
                    Date.now() - activeParticipants[stream.name][participant.id] < 2 * 60 * 1000);
                const isStillActiveByName = Boolean(clientsByIdentity[participant.userName] &&
