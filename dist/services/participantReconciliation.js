    ...
    try {
        ...
        for (const stream of activeStreams) {
                ...
                for (const participantId of activeIds) {
                    ...
                    if (now - lastActivity > 5 * 60 * 1000) { // 5 minutes
                        ...
                        if (participant && !participant.leftAt) {
                            await ParticipantManager.markParticipantAsLeft(stream.name, participant.walletAddress, participantId);
            ...
            for (const participant of participantsToCheck) {
                const isStillActive = Boolean(clientsByIdentity[participant.id] &&
