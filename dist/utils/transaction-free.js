 ...
 */
export async function executeWithCompensation(actions) {
    const completed = [];
    const errors = [];
    let failedIndex = -1;
...
export class DistributedLock {
    async acquire(key, ttlSeconds = 30) {
        ...
        try {
            await executeQuery(() => db.idempotencyKey.create({
                data: {
                    key: lockKey,
            ...
            if (error.code === 'P2002') {
                ...
                if (existing && existing.expiresAt < new Date()) {
                    ...
                    try {
                        await executeQuery(() => db.idempotencyKey.delete({
                            where: { key: lockKey }
                        }), { maxRetries: 1, timeout: 1000 });
 ...
 */
export async function updateWithOptimisticLock(model, where, data, maxRetries = 3) {
    for (let attempt = 0; attempt < maxRetries; attempt++) {
        try {
            // Get current record with version
            const current = await executeQuery(() => model.findUnique({
                where,
            ...
            const result = await executeQuery(() => model.updateMany({
                where: {
                    ...where,
            ...
            if (result.count > 0) {
                ...
                const updated = await executeQuery(() => model.findUnique({ where }), { maxRetries: 1, timeout: 2000 });
