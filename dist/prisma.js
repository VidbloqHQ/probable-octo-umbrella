...
const prismaClientSingleton = () => {
    ...
    if (!databaseUrl.includes('pgbouncer=true')) {
        const separator = databaseUrl.includes('?') ? '&' : '?';
        // Increased connection_limit from 10 to 30
        databaseUrl = `${databaseUrl}${separator}pgbouncer=true&connection_limit=30&pool_timeout=10&statement_timeout=10000`;
    }
    console.log('Initializing Prisma Client', {
        environment: process.env.NODE_ENV || 'development',
        connectionPool: 'PgBouncer Transaction Mode',
        connectionLimit: '30', // Increased
        statementTimeout: '10 seconds'
    });
    return new PrismaClient({
...
// REMOVE THE QUEUE ENTIRELY - it's causing your bottleneck
// Direct execution with retry logic only
export async function executeQuery(queryFn, options = {}) {
    const { maxRetries = 2, timeout = 5000, retryDelay = 500 } = options;
    let lastError;
    for (let attempt = 0; attempt < maxRetries; attempt++) {
        try {
            ...
            lastError = error;
            trackQuery(false, error);
            // Log connection pool errors
            if (error.code === 'P2024') {
                console.error(`[Connection Pool] Exhausted at attempt ${attempt + 1}`);
            }
            // Retryable error codes
            const retryableCodes = ['P2024', 'P2034', 'P1001', 'P1002', 'TIMEOUT'];
            const shouldRetry = retryableCodes.includes(error.code) ||
                error.message === 'Query timeout';
...
// No transactions with PgBouncer
export async function executeTransaction(fn, options) {
    console.warn('[WARNING] Transactions not supported with PgBouncer in transaction mode');
    return fn(db);
}
// Health check
...
// Simple query stats (no queue to track)
export function getQueueStats() {
    return {
        active: 0,
        queued: 0,
        maxConcurrent: 'unlimited',
        note: 'Queue removed - direct execution'
    };
}
// Query tracking
let queryCount = 0;
...
export function trackQuery(success, error) {
    ...
    const now = Date.now();
    if (now - lastReset > 3600000) {
        console.log(`[Stats] Hour: ${queryCount} queries, ${errorCount} errors (${((errorCount / queryCount) * 100).toFixed(2)}%)`);
        queryCount = 0;
        errorCount = 0;
        poolExhaustedCount = 0;
        lastReset = now;
