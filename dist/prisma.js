...
export async function executeQuery(queryFn, options = {}) {
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
        try {
            ...
            const delay = Math.min(retryDelay * Math.pow(2, attempt) + Math.random() * 500, 10000);
...
export async function executeTransaction(fn, options = {}) {
    const { maxWait = 5000, timeout = 30000, isolationLevel, retries = 3 } = options;
    return executeQuery(() => db.$transaction(fn, { maxWait, timeout, isolationLevel }), { maxRetries: retries, timeout: timeout + 5000 } // Give extra time for transaction
    );
}
// Optimized health check
export async function isDatabaseHealthy() {
    try {
        await executeQuery(() => db.$queryRaw `SELECT 1 as healthy`, { maxRetries: 1, timeout: 5000 });
        return true;
    }
    catch (error) {
...
export function trackQuery(success) {
    queryCount++;
    if (!success)
        errorCount++;
