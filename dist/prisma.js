...
export async function executeQuery(queryFn, options = {}) {
    const { maxRetries = 2, // Reduce retries for faster failures
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
        try {
            ...
            const delay = Math.min(retryDelay * Math.pow(2, attempt) + Math.random() * 200, 5000 // Max 5 seconds
            );
...
export async function executeTransaction(fn, options = {}) {
    const { maxWait = 5000, timeout = 15000, // Reduce from 30s
    isolationLevel, retries = 2 // Reduce retries
     } = options;
    return executeQuery(() => db.$transaction(fn, { maxWait, timeout, isolationLevel }), { maxRetries: retries, timeout: timeout + 5000 });
}
// Optimized health check
export async function isDatabaseHealthy() {
    try {
        await executeQuery(() => db.$queryRaw `SELECT 1 as healthy`, { maxRetries: 1, timeout: 3000 } // Very quick health check
        );
        return true;
    }
    catch (error) {
...
export function trackQuery(success, duration) {
    queryCount++;
    if (!success)
        errorCount++;
