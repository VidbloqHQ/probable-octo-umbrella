...
export async function executeQuery(queryFn, options = {}) {
    const { maxRetries = 2, timeout = 10000, retryDelay = 500 } = options;
    let lastError;
...
export async function executeTransaction(fn, options = {}) {
    const { maxWait = 2000, // Reduced from 5000
    timeout = 10000, // Reduced from 15000
    isolationLevel, retries = 1 // Reduced from 2 - transactions shouldn't retry much
     } = options;
...
setInterval(async () => {
    try {
        ...
        const idleCheck = await db.$queryRaw `
    ...
    `;
        const idleCount = idleCheck[0].idle_count;
        if (idleCount > 0) {
