...
class UltraAuthCache {
    cache = new Map();
    secretCache = new Map(); // Cache bcrypt results
    hits = 0;
    misses = 0;
    ...
    setSecretVerification(apiKey, apiSecret, isValid) {
        const secretKey = `${apiKey}:${apiSecret}`;
...
async function prewarmCache() {
    try {
        ...
        const recentTokens = await executeQuery(() => db.apiToken.findMany({
            where: {
                isActive: true,
                lastUsedAt: {
        ...
        if (recentTokens.length > 0) {
            ...
            const tenants = await executeQuery(() => db.tenant.findMany({
                where: {
                    id: { in: tenantIds }
                },
                select: {
...
export const authenticateTenant = async (req, res, next) => {
    ...
    try {
        ...
        const result = await executeQuery(() => db.apiToken.findUnique({
            where: { key: apiKey },
            select: {
                id: true,
                secret: true,
